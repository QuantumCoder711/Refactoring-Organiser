        ...
        <div className="flex gap-2 flex-col w-full" ref={dropdownRef}>
            ...
            <div className="relative">
                <div className="relative">
                    ...
                    />
                    {/* You may want to use a chevron icon here if available */}
                    <ChevronDown
                            ...
                            filteredOptions.map((option) => (
                                ...
                                >
                                    ...
                                    {inputValue === option.name && (
                                        <span className="size-4 text-secondary">✔</span>
                                    )}
                                </div>
                            ))
                        ) : searchTerm ? (
...
const AddFirstEvent: React.FC = () => {
    ...
    const handleCreateAccount = async () => {
        const { first_name, last_name, email, mobile_number, password, confirm_password, company_name, tnc, notifications } = userAccount;
        ...
        try {
            ...
            if (res.data.status === 200) {
                ...
                if (loginResponse.status === 200) {
                    ...
                    const response = await useEventStore.getState().addEvent(finalFormData, loginResponse.access_token);
                    if (response.status === 200) {
                        toast("Event added successfully", {
                            className: "!bg-green-800 !text-white !font-sans !font-regular tracking-wider flex items-center gap-2",
                            icon: <CircleCheck className='size-5' />
    ...
    return (
        <React.Fragment>
            ...
            <div className='min-h-full min-w-full p-3'>
                    ...
                    <DialogContent className='bg-muted'>
                        <DialogHeader>
                            ...
                            {userAccount.step == "1" && <div>
                                ...
                                <div className='flex gap-5 justify-between mt-5'>
                                    <div className='flex gap-2 flex-col w-full'>
                                        {/* Company Name ComboBox from Signup */}
                                        <CustomComboBox
                                            label="Company Name"
                                            ...
                                            options={companies.map((company: CompanyType, index: number) => ({ id: index + 1, name: company.company }))}
                                            required
                                        />
                                    </div>
                                </div>
                ...
                <div className='max-w-[700px] rounded-[10px] !mx-auto p-5 lg:p-8 bg-muted mt-5'>
                    {/* Event Name */}
                        ...
                        <div className='flex justify-between gap-3 mt-5 w-full overflow-x-scroll'>
                            {templates.map((template, index) => (
                                <img
                                    ...
                                    width={100}
                                    height={100}
                                    className={`size-24 object-cover rounded-[10px] cursor-pointer hover:border-2 hover:border-primary transition-all ${selectedTemplate === template ? 'border-2 border-primary' : ''}`}
                                />
                            ))}
                        </div>
                    )}
